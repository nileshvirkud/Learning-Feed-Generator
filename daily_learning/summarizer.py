...
{{
    ...
    def summarize_article(self, article: Article, sentence_count: int = 4) -> Optional[Summary]:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert educational content curator who creates concise, valuable summaries for learners."},
            ...
            try:
                ...
                logger.error(f"Failed to parse JSON response for article: {article.title}")
    ...
    def summarize_batch(self, articles: List[Article], sentence_count: int = 4) -> List[Summary]:
        """Summarize multiple articles with error handling"""
        summaries = []
        
        for i, article in enumerate(articles):
            logger.info(f"Summarizing article {i+1}/{len(articles)}: {article.title[:50]}...")
...
{{
    ...
    "related_topics": ["Related topic 1", "Related topic 2"]
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an educational expert who enhances content for optimal learning."},
            ...
            logger.info(f"Enhanced summary for: {summary.original_article.title[:50]}...")
            return summary
    ...
    def get_content_quality_score(self, summary: Summary) -> float:
        ...
        educational_keywords = ['learn', 'understand', 'concept', 'method', 'technique', 'principle', 'skill']
        summary_lower = summary.summary.lower()
        keyword_count = sum(1 for keyword in educational_keywords if keyword in summary_lower)
